...
public interface CommodityMapper extends Mapper<Commodity> {
	...
	@Results({ @Result(column = "id", property = "id"),
			...
			@Result(column = "id", property = "commodityTypeIds", many = @Many(select = "main.dao.CommodityTypeMapper.gainTypeByCommodityId")),
			@Result(column = "id", property = "commodityDesigns", many = @Many(select = "main.dao.CommodityDesignMapper.gainDesignByCommodityId")) })
	...
	// 获取所有商品
		@Results({ @Result(column = "id", property = "id"),
				...
				@Result(column = "id", property = "commodityTypeIds", many = @Many(select = "main.dao.CommodityTypeMapper.gainTypeByCommodityId")),
				@Result(column = "id", property = "commodityDesigns", many = @Many(select = "main.dao.CommodityDesignMapper.gainDesignByCommodityId")) })
		@Select("SELECT * FROM tb_commodity WHERE id = #{i}")
		public List<Commodity> caol(Integer i);
